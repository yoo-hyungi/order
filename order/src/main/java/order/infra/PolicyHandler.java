...
public class PolicyHandler {
    ...
    @StreamListener(
        value = KafkaProcessor.INPUT,
        condition = "headers['type']=='ProductIncreased'"
    )
    public void wheneverProductIncreased_NotifyWaitCustomer(
        @Payload ProductIncreased productIncreased
    ) {
        ProductIncreased event = productIncreased;
        System.out.println(
            "\n\n##### listener NotifyWaitCustomer : " +
            productIncreased +
            "\n\n"
        );

        // Sample Logic //
        Order.notifyWaitCustomer(event);
    }
}
//>>> Clean Arch / Inbound Adaptor
